function crncatalytic(concA, concB, stopTime, figNo)
    ...
    % Add reaction set X +Y -> Z + Y to the solver
    r1 = addreaction(model,'A + Gab -> Ia');
    r2 = addreaction(model,'B + Gba -> Ib');
    r3 = addreaction(model,'A + Ib -> Iao + W');
    r4 = addreaction(model,'B + Ia -> Ibo + W');
    r5 = addreaction(model,'Iao + Gbc -> C + B');
    r6 = addreaction(model,'Ibo + Gbc -> C + B');

    % Add waste reactions to the solver
    r7 = addreaction(model,'A + Gba -> W');
    r8 = addreaction(model,'B + Gab -> W');
    ...
    kl7 = addkineticlaw(r7, 'MassAction');
    kl8 = addkineticlaw(r8, 'MassAction');
    ...
    p7 = addparameter(kl7, 'c7', 'Value', rateK);
    p8 = addparameter(kl8, 'c8', 'Value', rateK);
    ...
    kl7.ParameterVariableNames = {'c7'};
    kl8.ParameterVariableNames = {'c8'};
    ...
    r5.Reactants(2).InitialAmount = baseConc;             % Gate bc
    ...
    plot(t, X(:, 11), 'LineWidth', 2.0);
    title('Polymerase-based catalytic CRN');
